...
public class CountedDataDistribution<KeyType> extends
  ...
  public double adjust(KeyType key, final double value, int count) {
    ...
    if (entry == null) {
      ...
      if (this.isLogScale) {
        if (count < 0)
          sum = LogMath2.subtract(entry.value, value);
        else
          sum = LogMath2.add(entry.value, value);
      } else {
    ...
    if (this.isLogScale) {
      if (count < 0) 
        this.total = LogMath2.subtract(this.total, value);
      else
        this.total = LogMath2.add(this.total, value);
    } else {
      this.total += delta;
    }
  ...
  public void set(final KeyType key, final double totalValue,
    ...
    if (entry == null) {
      ...
      if (totalValue > identity) {
        ...
        if (this.isLogScale) {
          this.total = LogMath2.add(this.total, totalValue);
        } else {
      ...
      if (this.isLogScale) {
        if (entry.value > totalValue) {
          final double totalsSum =
              LogMath2.add(this.total, totalValue);
          Preconditions.checkState(totalsSum >= entry.value);
          this.total = LogMath2.subtract(totalsSum, entry.value);
        } else {
          this.total =
              LogMath2.add(this.total,
                  LogMath2.subtract(totalValue, entry.value));
        }
      } else {
